...
export default function RunwayCard({ data }) {
  ...
  const getRunwayInfo = (months) => {
    if (months >= 12) return { color: 'text-green-600', bg: 'bg-green-500', label: 'Seguro', emoji: '🟢', icon: Shield };
    if (months >= 6) return { color: 'text-yellow-600', bg: 'bg-yellow-500', label: 'Razoável', emoji: '🟡', icon: Clock };
    if (months >= 3) return { color: 'text-orange-600', bg: 'bg-orange-500', label: 'Atenção', emoji: '🟠', icon: Clock };
    return { color: 'text-red-600', bg: 'bg-red-500', label: 'Emergência', emoji: '🔴', icon: Clock };
  };
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 pt-4 border-t">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-900">{formatCurrency(avgMonthlyExpenses)}</span>
            </div>
          </div>
          ...
          <div className={`${
            ...
            runwayMonths >= 3 ? 'bg-orange-50 border-orange-200' :
            'bg-red-50 border-red-200'
          } border rounded-lg p-3`}>
            <p className={`text-xs ${
              runwayMonths >= 12 ? 'text-green-800' :
              runwayMonths >= 6 ? 'text-yellow-800' :
              runwayMonths >= 3 ? 'text-orange-800' :
              'text-red-800'
            }`}>
              💡 {runwayMonths >= 12
