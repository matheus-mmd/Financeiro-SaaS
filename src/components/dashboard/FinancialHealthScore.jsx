...
export default function FinancialHealthScore({ score, breakdown }) {
  const getScoreColor = (score) => {
    if (score >= 80) return { color: 'text-green-600', bg: 'bg-green-500', label: 'Excelente', emoji: '🟢' };
    if (score >= 60) return { color: 'text-yellow-600', bg: 'bg-yellow-500', label: 'Bom', emoji: '🟡' };
    if (score >= 40) return { color: 'text-orange-600', bg: 'bg-orange-500', label: 'Atenção', emoji: '🟠' };
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">
                ...
                {score >= 60 && score < 80 && 'Bom trabalho! Há espaço para melhorar.'}
                {score >= 40 && score < 60 && 'Atenção necessária. Faça ajustes.'}
          ...
          {breakdown && (
            <div className="space-y-2 pt-4 border-t">
              <p className="text-xs font-medium text-gray-500 uppercase">Componentes do Score</p>

              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${breakdown.balancePositive ? 'bg-green-500' : 'bg-red-500'}`} />
                  <span className="text-gray-600">Saldo positivo</span>
                </div>

                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${breakdown.savingsRate ? 'bg-green-500' : 'bg-red-500'}`} />
                  <span className="text-gray-600">Taxa poupança</span>
                </div>

                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${breakdown.expenseRatio ? 'bg-green-500' : 'bg-red-500'}`} />
                  <span className="text-gray-600">Despesas controladas</span>
                </div>

                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${breakdown.growthTrend ? 'bg-green-500' : 'bg-red-500'}`} />
